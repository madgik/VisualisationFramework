...
public class GraphVisualizationHelper {
	...
	public static Map<String, Object> nodesToD3Format(Collection<Node> nodeEntities, boolean isInitialization) {
		...
		while (result.hasNext()) {
			...
			Map<String, Object> nodeMap = map("id", node.getNodeId(), "latitude", node.getX(), "longitude", node.getY(), "size", node.getSize());
	...
	public static Map<String, Object> hasWeightToD3Format(Collection<HasWeight> nodeEntities, String graphId, NetworkGraphService networkGraphService) {
		...
		while (result.hasNext()) {
			...
			if (existedNodes.get(node.getNodeId()) == null) {
				...
				Map<String, Object> nodeMap = map("id", node.getNodeId(), "value", hasWeight.getSource().getProperty(), "size", node.getSize()); // "x", node.getX(), "y", node.getY(),
				for (NodeProperty property : node.getNodeProperties()) {
					nodeMap.put(property.getName(), property.getValue());
				}
				nodes.add(nodeMap);
		...
		targetIds.entrySet().stream().forEach(t -> {
			if (existedNodes.get(t) == null) {
				...
				if (node != null) {
					...
					Map<String, Object> nodeMap = map("id", node.getNodeId(), "value", t.getValue(), "size", node.getSize()); // "x",  node.getX(), "y"
					for (NodeProperty property : node.getNodeProperties()) {
						nodeMap.put(property.getName(), property.getValue());
					}
	...
	public static List<Node> resizeNodesWithLinks(List<TopNodesResult> nodes) {
